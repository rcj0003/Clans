...
public class ForceDemoteCommand extends ArgumentSubCommand {
	private static final String[] DESCRIPTION = new String[] { "Force demotes a player to next available role." };
	private static final Argument<?>[] arguments = new Argument<?>[] { new PlayerArgument() };
	private Plugin plugin;
	private ClanService clanService;
	private MessageConfiguration config;
	
	public ForceDemoteCommand(Plugin plugin, ClanService clanService, MessageConfiguration config) {
		this.plugin = plugin;
	...
	public void executeVerified(CommandUser user, Object[] values) {
		...
		new BukkitRunnable() {
			public void run() {
				...
				ClanRole newRole = promotee.getRole().getNextLowerRole();
				Bukkit.getPluginManager().callEvent(new ClanDemoteEvent(clan, promotee, promotee.getRole(), newRole));
