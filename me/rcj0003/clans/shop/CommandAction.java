...
public class CommandAction implements PurchaseAction {
	...
	public CommandAction(Plugin plugin, ClanService clanService, SQLHelper sqlHelper, String internalName, String command, int price, long cooldownTime) {
		this.plugin = plugin;
		...
		try (Connection connection = sqlHelper.openConnection()) {
			PreparedStatement statement = connection.prepareStatement("CREATE TABLE IF NOT EXISTS (id CHAR(36), internal_name VARCHAR(64), expiration BIGINT);");
	...
	public void purchase(Player player) {
		...
		Bukkit.getConsoleSender().sendMessage(command.replace("%player_name%", player.getDisplayName()));
	}
